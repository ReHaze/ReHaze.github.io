document.addEventListener("DOMContentLoaded", function(event) {
  ...
  let window_success_button = document.querySelector(
    ".form-validation-success__close-button"
  );
  let window_fail_button = document.querySelector(
    ".form-validation-fail__close-button"
  );
  form.addEventListener("submit", function(e) {
    let values = Array.from(document.querySelectorAll("input"));
    let name = values[0];
    let lastName = values[1];
    let secondaryName = values[2];
    let phone = values[3];
    let mail = values[4];
    name.style.border = "";
